...
/**
 * Retorna todos los productos de una unidad productiva / reserva.ejs
 * @param {Number} up  Código de la unidad productiva
...
/**
 * Función que crea un movimiento al usuario y retorna el id / reserva.ejs
 * @param {Number} punto_venta  ID del punto de venta
 * @returns {JSON} Retorna JSON con el id del movimiento
 */
...
/**
 * Función que lista las reservas / historial-reservas.ejs - reservas.ejs
 * @param {Number} limite Límite de datos a listar
 * @returns {JSON} Retorna JSON con las reservas
 */
...
/**
 * Lista los usuarios activos de una ficha / reservas.ejs
 * @param {Number} idFicha Id de la ficha
 * @returns {JSON} Retorna JSON con los usuarios
 */
controlador.Listar_Usuaios_Ficha = async(req, res) => {
    let sql = `SELECT identificacion, Nombres 
    FROM personas 
    where Ficha = '${req.body.idFicha}' and Estado = 1`
    try {
        let rows = await query(sql);
...
/**
 * Lista las unidades productivas que tienene reservas / reserva.ejs
 * @returns {JSON} Retorna JSON con las unidades productivas
 */
controlador.Lista_Unidades_Reserva = async(req, res) => {
    try {
        let sql = `SELECT DISTINCT fk_codigo_up as up_id, up.Nombre as nombre 
        FROM productos p
...
/**
 * Registra detalle al usuario / reserva.ejs
 * @param {Number} persona       Identificacion de la persona
 * @param {Number} cantidad      Cantidad del producto a reservar
...
/**
 * Función que elimina un detalle que no está facturado
